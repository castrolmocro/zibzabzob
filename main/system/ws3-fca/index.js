  ...
  mainPromise
    .then(async (res) => {
      ...
      if (!_keepAliveInitialized) {
        ...
        setInterval(saveAppState, 5 * 60 * 1000);

        const doCookieRefreshPing = async () => {
          try {
            const res = await utils.get("https://www.facebook.com/", _keepAliveJar, null, _keepAliveGlobalOptions, { noRef: true });
            utils.saveCookies(_keepAliveJar)(res);
            saveAppState();
            logger(`cookies refreshed and appstate saved successfully`, "cookie");
          } catch (e) {
            logger.warn("cookie refresh ping failed: " + (e.message || e));
          }
        };

        setInterval(doCookieRefreshPing, 20 * 60 * 1000);

        setInterval(() => {
          try {
            if (api && typeof api.refreshFb_dtsg === 'function') {
              api.refreshFb_dtsg((err, result) => {
                if (err) return logger.warn("fb_dtsg refresh failed: " + (err.message || err));
                logger(`fb_dtsg token refreshed successfully`, "cookie");
              });
            }
          } catch (e) {
            logger.warn("fb_dtsg refresh error: " + (e.message || e));
          }
        }, 24 * 60 * 60 * 1000);
