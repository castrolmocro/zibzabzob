  ...
  function autoDeleteCache(config) {
    if(config.status) {
      setInterval(async () => {
        ...
        const cacheCmds = `${__dirname}/scripts/commands/cache`;
        const cacheEvts = `${__dirname}/scripts/events/cache`;
        exec(`rm -rf "${cacheCmds}" && mkdir -p "${cacheCmds}" && rm -rf "${cacheEvts}" && mkdir -p "${cacheEvts}"`, (error, stdout, stderr) => {
        if (error) {
